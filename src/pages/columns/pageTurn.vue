<template>
    <div class="contentbox" id="container">
        <div class="columnbox" id="content" v-html="html"></div>
    </div>
</template>

<script>
const html = '<li>【荷塘月色】</li>'
            +'<li>这几天心里颇不宁静。今晚在院子里坐着乘凉，忽然想起日日走过的荷塘，在这满月的光里，总该另有一番样子吧。月亮渐渐地升高了，墙外马路上孩子们的欢笑，已经听不见了;妻在屋里拍着闰儿，迷迷糊糊地哼着眠歌。我悄悄地披了大衫，带上门出去。</li>'
    ...
    methods: {
        scroll(){
            ...
            con.addEventListener('mousedown',function(event){
                points.startPoint = event.offsetX;
            });
            con.addEventListener('mouseup',function(event){
                ...
                if( points.endPoint < points.startPoint ){
                    ...
                    if(con.scrollWidth > content.offsetWidth){
                        points.left ++;
                        points.right --;
                        content.style.transform = 'translateX('+ -400*points.left +'px)';
                    }else{
                        alert('已经结束');
                    }
                    ...
                    if(points.left <= 0){
                        ...
                        points.right ++;
                        points.left --;
                        content.style.transform = 'translateX('+ 400*points.right +'px)';
                    }
                }
            });
        }
